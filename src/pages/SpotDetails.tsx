    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base flex items-center gap-2">
          <MessageCircle className="h-5 w-5 text-primary"/>Relatos do Dia
...
export default function SpotDetails() {
  ...
  useEffect(()=>{
    if(id){
      fetchCurrentConditions().then(async spots=>{
        ...
        setSpot(found); setLoadingSpot(false); setTimeout(()=>setVisible(true),50)
        if(found){
          getWeatherForecast(found.id,{waveHeight:found.waveHeight,windSpeed:found.windSpeed,swellPeriod:found.swellPeriod,windDirection:found.windDirection,waterTemperature:found.waterConditions.temperature,score:found.score},isPremium).then(setForecast)
        }
      })
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ✅ Header com scroll horizontal no mobile */}
      <header className="sticky top-0 z-40 bg-card/80 backdrop-blur-md border-b border-border/40">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide" style={{scrollbarWidth:'none'}}>
            ...
            <div className="flex-1"/>
            <Button variant="outline" size="sm" onClick={()=>navigate('/navigation')} className="flex-shrink-0">
              <Navigation className="h-4 w-4 mr-1.5"/>GPS
            </Button>
            <ShareButton spot={spot}/>
            <Button variant={favorite?'default':'outline'} size="sm" onClick={handleToggleFavorite} disabled={loadingFav} className="flex-shrink-0">
              <Heart className={`h-4 w-4 mr-1.5 ${favorite?'fill-current':''}`}/>{favorite?'Favoritado':'Favoritar'}
            </Button>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-6 max-w-4xl space-y-6">
        ...
        {/* ✅ Só 2 abas: Agora e Previsão (Câmera removida) */}
                            ...
                            {selectedSubRegion?.id===subRegion.id&&(
                              <div className="mt-3 pt-3 border-t border-border/40 space-y-2" style={{animation:'slideUp 0.2s ease-out'}}>
                                <div className="grid grid-cols-3 gap-2 text-center">
                                  <div className="bg-background/60 rounded-lg p-2"><div className="text-xs text-muted-foreground">Ondas</div><div className="text-sm font-bold text-primary">{waveMin}m - {waveMax}m</div></div>
                                  <div className="bg-background/60 rounded-lg p-2"><div className="text-xs text-muted-foreground">Período</div><div className="text-sm font-bold">{Math.round(spot.swellPeriod)}s</div></div>
                                  <div className="bg-background/60 rounded-lg p-2"><div className="text-xs text-muted-foreground">Vento</div><div className="text-sm font-bold">{Math.round(spot.windSpeed)}km/h</div></div>
                                </div>
                                <div className="text-xs text-muted-foreground bg-background/40 rounded-lg p-2">
                                  <span className="font-semibold">{swellMatch}</span>
                                  {minDiff===0?` — Swell de ${spot.swellDirection} é ideal para este pico.`:minDiff<=2?` — Swell de ${spot.swellDirection} funciona bem aqui. Ideal seria ${idealDirs.join(' ou ')}.`:` — Este pico funciona melhor com swell de ${idealDirs.join(', ')}.`}
                                </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  {!isPremium&&<Badge variant="outline" className="ml-auto text-xs text-yellow-500 border-yellow-500/40">3 dias grátis · <Crown className="h-3 w-3 inline mx-0.5"/> Premium = 14 dias</Badge>}
                </CardTitle>
              </CardHeader>
              <CardContent>
                {forecast.length===0?(
                  ...
                  <div className="space-y-3">
                    {forecast.map((day,index)=>{
                      ...
                      // ✅ FREE_DAYS = 3
                      const isLocked=(index>=FREE_DAYS)&&!isPremium
                      return (
                        <div key={day.date}
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            {isLocked?(
                              <div className="flex items-center gap-2 text-muted-foreground"><Lock className="h-4 w-4"/><span className="text-sm">Exclusivo Premium</span></div>
                            ):(
